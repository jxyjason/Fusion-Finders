...
def eval(segments, data):#输出评估项：是否大于阈值、mean iou
    tious = [float(i) for i in config.TEST.TIOU.split(',')] if isinstance(config.TEST.TIOU,str) else [config.TEST.TIOU]#存储两个值
    recalls = [int(i) for i in config.TEST.RECALL.split(',')] if isinstance(config.TEST.RECALL,str) else [config.TEST.RECALL]#存储两个值

    eval_result = [[[] for _ in recalls] for _ in tious]#创建空列表，大小为recalls列tious行
    max_recall = max(recalls)
    average_iou = []
    for seg, dat in zip(segments, data):#合并预测值与真值，计算nms、iou和平均iou
        ...
        for i,t in enumerate(tious):#计算输出第一项，表示iou是否大于阈值:依次对rank1/5，mIoU0.5/0.7计算
...
def eval_predictions(segments, data, verbose=True):#读取预测值和真值，并将eval评估得到的结果输出
