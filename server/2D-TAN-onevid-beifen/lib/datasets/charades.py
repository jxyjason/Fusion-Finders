...
class Charades(data.Dataset):
    ...
    def __init__(self, split):
        ...
        self.durations = {}#存储视频持续时间--来自csv文件
        ...
        annotations = []#存储视频编号、相应描述的开始结束时间、描述、整个视频持续时间--来自txt文件annotations.append({'video':vid, 'times':[s_time, e_time], 'description': sent, 'duration': self.durations[vid]})
        for line in anno_file:
            anno, sent = line.split("##")
            sent = sent.split('.\n')[0]
            vid, s_time, e_time = anno.split(" ")
            s_time = float(s_time)
            e_time = min(float(e_time), self.durations[vid])#如果结束时间大于视频时长则换为视频时长
            if s_time < e_time:#如果换为视频时长后结束时间大于开始时间，则存储
