...
global device,model,bert_model

def pre_lavt():
    print('-----------initial LAVT-----------')
    global device,model,bert_model
    device = ("cuda" if torch.cuda.is_available() else "cpu" )
    weights = 'LAVT-RIS-main/checkpoints/refcoco.pth'
...
def beginDetection(img,original_w,original_h,sentence):
    global device,model,bert_model
    # begin
    ...
    for i in range(original_h):
        for j in range(original_w):
            for k in range(3):
                rgbResult+=(str(visualization[i][j][k]))+','
    rgbResult+=']'
    return rgbResult
    ...
    # visualization.save('LAVT-RIS-main/demo/demo_result.jpg')
